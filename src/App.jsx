...
const S = {
  ...
  eraPeriod: { fontSize: "11px", fontFamily: "monospace", color: "#5A4A38" },
  formWrap: {
    maxWidth: "480px",
    margin: "0 auto",
    display: "flex",
...
[Content]`
      ...
      // STEP 2: Storyboard
      setStage("storyboard");
      setStatusMsg("Building storyboard…");
...
${script}`
      ...
      // STEP 3: Video generation with delay between shots
      setStage("video");
      const urls = [];

      for (let i = 0; i < shotLines.length; i++) {
        setStatusMsg(`Generating shot ${i + 1} of ${shotLines.length}…`);

        // Wait 15s between shots to avoid rate limiting
        if (i > 0) {
          setStatusMsg(`Waiting before shot ${i + 1}…`);
          await sleep(15000);
        }
      ...
      // STEP 4: Synopsis
      setStage("final");
      setStatusMsg("Writing final synopsis…");
      ...
      <div style={S.hero}>
        ...
        {stage && era && (
          <div style={S.eraBadge(accent, era.surface)}>
            ...
            {statusMsg && <span style={S.statusMsg}>{statusMsg}</span>}
          </div>
        )}
